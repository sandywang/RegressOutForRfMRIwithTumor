function Run_RegressOut(InputFile, TMMsk, GSMsk, WMMsk, CSFMsk, HMInd, HMFile)
%-------------------------------------------------------------------------%
%   RUN Regress Out Covariates
%   Input:
%   InputFile    - The input file list, Nx1 cell (1x1 cell for 4D NIfTI)
%   TMMsk        - The mask of tumor
...
% Tumor Mask
V_TMMsk=spm_vol(TMMsk);
Y_TMMsk=spm_read_vols(V_TMMsk);
Y_TMMsk=reshape(Y_TMMsk, [], 1);
Y_NoTMMsk=~Y_TMMsk;
...
switch HMInd
    ...
    case 4 %36 Derivative
        Rp=load(HMFile{1});
        Dif=diff(Rp);
        Dif=[zeros(1,6); Dif];
        HMCov=[Rp, Rp.^2, Dif, Dif.^2];
        
        ImgCovDif=diff(ImgCov);
        ImgCovDif=[zeros(1, size(ImgCov, 2)); ImgCovDif];
        ImgCov=[ImgCov, ImgCovDif, ImgCov.^2, ImgCovDif.^2];
end

% Regressor
Rgr=ones(TP, 1);
Rgr=[Rgr, (1:TP)', ImgCov, HMCov];
